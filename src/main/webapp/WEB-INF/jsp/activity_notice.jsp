...
<div id="sign-situation">
    <form class="layui-form">
        <div class="sign">
            ...
            <div id="sign_excel">
                <button class="layui-btn" style="height: 51px;" id="export">导出Excel</button>
            </div>
        </div>
        ...
        <div class="sign_up_body">
            <form role="form" class="sign_up_form" method="post" action="">
                ...
                <div class="form-group" id="s_name">
                    <input type="text" class="form-control s_name" placeholder="请输入姓名" value="${student.s_name}"
                           name="s_name"/>
                    <i class=""></i>
                </div>
                <div class="form-group" id="s_no">
                    <input type="text" class="form-control s_no" placeholder="请输入学号" value="${student.s_num}"
                           name="s_no"/>
                </div>
                <div class="form-group" id="s_class">
                    <input type="text" class="form-control s_class" placeholder="请输入班级" value="${student.s_class}"
                           name="s_class"/>
                </div>
                <div class="form-group" id="s_sex">
                    <input type="text" class="form-control s_sex" placeholder="请输入性别" value="${student.s_sex}"
                           name="s_sex"/>
                </div>
                <div class="form-group" id="s_acd">
                    <input type="text" class="form-control s_acd" placeholder="请输入学院" value="${student.s_acd}"
                           name="s_acd"/>
                </div>
                <div class="form-group" id="s_phone">
                    <input type="text" class="form-control s_phone" placeholder="请输入电话" value="${student.s_tel}"
                           name="s_phone"/>
                </div>
                <div class="form-group" id="s_email">
                    <input type="email" class="form-control s_email" placeholder="请输入邮箱" value="${student.s_mail}"
                           name="s_email"/>
                </div>
...
<script>
    ...
    function ExportExcel() {
        ...
        var a_id = $("#sign-situation tr:eq(0) th:eq(0)").text();

        $.ajax({
            type: "post",
            url: "/activitySign/exportExcel",
            data: {
                "a_id": a_id
            },
            dataType: "json",
...
<script>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $.each(data, function (i, data) {
                            ...
                            a_name = data.a_name;
                            var $tr = '<tr><td>' + (data.s_name) + '</td><td>' + (data.s_num) + '</td><td>' + (data.s_class) + '</td><td>' + (data.s_acd) + '</td></tr>';
                            $("#sign-situation").find("tbody").append($tr);

                        });

                        $("#sign-situation tr:eq(0) th:eq(1)").html("<b>" + a_name + "</b>")

                        $("#sign-situation tr:eq(0) th:eq(0)").html("<b>" + a_id + "</b>")
                    }
                });
            }
    ...
    function SignIn(obj) {
        ...
        $(".submit").click(function () {
            ...
            $.ajax({
                type: "post",
                url: "/activitySign/saveActivitySign?a_id=" + a_id,
                async: true,
